...
| 🔌 | **AI Agent 写入** | 内置 MCP Server，Claude Code / Cursor 等 AI 工具可直接在你的本地笔记库创建、追加、修改笔记 |
| 🔗 | **双向链接** | 输入 `[[` 引用其它笔记，单击跳转 + 自动反向链接 |
...
- **链接交互** — Ctrl+Click 打开外部链接（类似 VS Code）
- **笔记引用** — 输入 `[[` 引用其它笔记，单击跳转，被引用笔记自动显示反向链接（基于稳定 ID，跨设备同步有效）
- **自动保存** — 实时保存，永不丢失
...
- **5 项 KPI 一览** — 笔记总数、总字数、写作天数、活跃标签、连续天数，配色对应编辑器 5 高亮色
- **写作热力图** — 过去 90 天每日活跃度，颜色越深字数越多
- **最近 7 天趋势** — 折线图直观看到本周节奏
- **24h 时段分布** — 找出你的高产时段
- **Top 5 标签** — 最常用的标签使用频次
- **最近笔记** — 一键点击直接打开对应笔记
- **跟随应用主题** — 浅色/深色自动切换
