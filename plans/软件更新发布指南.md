...
> **⚠️ 本文已废弃，不要照做。** 以 `CLAUDE.md` 的「发布流程」为准。
> 本文写于手工打包时代，与现状冲突之处：应用已更名 Lapis；发布走 GitHub Actions（推 `v*` tag 触发），不再本地构建 + 手工 `gh release upload`；`latest.json` 由 tauri-action 生成；`pnpm signer:generate` 已移除；私钥不再放项目内的 `~/.tauri/`，改放仓库外的 `%USERPROFILE%\.tauri\lapis.key`。保留仅作历史记录。
