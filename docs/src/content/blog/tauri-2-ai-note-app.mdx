...
description: 分享使用 Tauri 2 + React + TypeScript 开发桌面笔记应用 Lapis 的技术选型、架构设计和开发经验，包括 AI 集成、MCP Server、SQLite 存储等实践。
keywords:
  - Tauri教程
  - Tauri 2开发
  - Tauri桌面应用
...
本文分享 [Lapis](https://github.com/zexadev/lapis) 的开发经验——一个基于 Tauri 2 + React 的桌面笔记应用，集成了 AI 助手、MCP Server、SQLite 存储等功能。
...
Lapis 支持多种 AI Provider（包括 GPT-5、Claude 4、DeepSeek V4、Gemini 3 等 2026 年主流模型），统一封装为相同的接口：
...
// tauri.conf.json
{
  "plugins": {
    "updater": {
      "endpoints": [
        "https://github.com/zexadev/lapis/releases/latest/download/latest.json"
      ]
    }
  }
}
...
Lapis 是一个用 Tauri 2 构建的真实项目，完整代码开源在 [GitHub](https://github.com/zexadev/lapis)，欢迎参考和贡献。

---

## 相关阅读

- [MCP 是什么？如何让 Claude Code 直接操作你的笔记](/blog/mcp-claude-code-notes) — 深入了解 Lapis 内置的 MCP Server 如何与 Claude Code 等 AI 工具协作。
- [2026 年本地笔记软件推荐：Obsidian vs 思源笔记 vs Lapis 深度对比](/blog/local-note-apps-comparison) — 从用户视角看 Tauri 构建的 Lapis 与 Electron 应用的实际差异。
- [Markdown 编辑器推荐 2026：支持 AI 辅助的 5 款最佳选择](/blog/markdown-editors-with-ai) — 了解 Lapis 在 Markdown 编辑器领域的定位和 AI 集成优势。

---

*想体验 Tauri 构建的应用？[下载 Lapis](https://github.com/zexadev/lapis/releases) 感受 8MB 安装包的轻量体验。*
