...
为什么越来越多开发者选择 **Tauri** 而不是 Electron 来构建桌面应用？一个数据就能说明：同样的笔记应用，Electron 打包 90MB+，Tauri 只需要 **8MB**。
...
| 数据库 | SQLite | 本地存储的最佳选择，通过 tauri-plugin-sql |
...
JD Notes 支持 5 种 AI Provider，统一封装为相同的接口：
...
MCP Server 提供三个工具（create_note、append_note、update_note），让 Claude Code 等 AI 工具可以直接操作笔记数据库。
...
Tauri 2 是 2026 年构建桌面应用的最佳选择之一。如果你已经有 React/Vue 前端经验，用 Tauri 可以快速构建出体积小、性能好的桌面应用。
