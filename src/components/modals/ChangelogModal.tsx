...
- **笔记多选同步**：点设备旁「选笔记」弹模态框，搜索/全选/单选要给对方的笔记
- **编辑器旁单条同步**：编辑器头部「推送」按钮直接发当前笔记给某设备
- **持久设备指纹**：从公钥派生 fingerprint，重启同设备在对方眼里仍是同一台
- **接收方提示**：对端推过来时自动 toast「收到「X」推来 N 条」
### 🌐 跨网 P2P 完整化
- 公司↔家不同网络加密直连（基于 iroh，NAT 打洞 + relay 兜底）
- 跨网设备列表 + 在线状态 + probe 自动取名
- 三路合并冲突保留双份，绝不静默丢数据
## 💅 优化
- 应用启动即注册 mDNS（无需打开同步页）
- 设备名变更立即生效（mDNS 重注册）
- 退出发 mDNS goodbye（对方立即清除本机）
- 设备同步页重设计：全局 toast + 分区布局
- 同步结果文案改双向人话
- 图片附件化存储（sha256 内容寻址）
## 🐛 修复
- 修复 local_ip 错选 VPN/Docker 网卡
- 修复 initDatabase 未调用导致 uuid 全空的「新增 0」假象
