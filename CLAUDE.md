...
- **iOS 构建**：只在 CI 出（`.github/workflows/ios.yml`，macos-latest；`workflow_dispatch` 可单独试跑挂 artifact，发版时 release.yml 在 android 之后以 workflow_call 调用）。Xcode 工程每次现场 `tauri ios init --ci` 生成、**不入库**（本机 Windows 没有 Xcode/xcodegen，维护不了）；不走 `tauri ios build`（必须 archive+export、没证书过不去），直接 `xcodebuild ... CODE_SIGNING_ALLOWED=NO build` 出 .app 打成 `Lapis_x.y.z_ios_unsigned.ipa` 供用户 AltStore/Sideloadly 自签。Info.plist 由 PlistBuddy 注入 NSLocalNetworkUsageDescription 与 NSBonjourServices（`_lapis._tcp`）。**没有 iOS 真机，全部只到编译级验证**；iOS 上 `isIOSPlatform` 分流：安全区走 CSS env()、键盘用 visualViewport 撑 `--safe-area-bottom`、更新只打开 IPA 地址
